...
' -- Thomas Gehrke
' -----------------------------------------------
' Adds account or group to local groups on
' domain workstations
' -----------------------------------------------
' RECORD OF REVISION:
' 08/15/2007 [tcg] Script rewritten.
' -----------------------------------------------
' USAGE:
'
' ===============================================
...
' *****************************************************
Dim strLogFolder
strLogFolder = objShell.SpecialFolders( "MyDocuments" ) & "\Add User to Local Group Logs\"
...
strLogFile = strLogFolder & "Add '[" & Replace(strTargetAccount,"/","]") & "' to '" & strTargetGroup & "' in " & strDomain & ".csv"
  ...
  If objItem.Class = "Computer" Then
    ...
    If Left(LCase(strComputer), LEN(strComputerNamePrefix)) = strComputerNamePrefix Then
      ...
      If err.number = 0 Then
        ...
        If err.number = 0 Then
          objTargetGroup.Add(objTargetAccount.ADsPath)

          If err.number = 0 Then
            objLogFile.WriteLine( """" & strComputer & """,""" & strTargetGroup & """,""" & strTargetAccount & """,""added"",""" & Now() & """" )
          End If
        End If
      End If
